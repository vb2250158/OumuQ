...
from app.workers.onnx_vits.engine import OnnxVitsEngine, write_pcm16_wav
...
class WorkerSettings:
    ...
    def from_env(cls) -> WorkerSettings:
        raw_providers = [
            item.strip() for item in os.environ.get("OUMUQ_ONNX_VITS_PROVIDERS", "").split(",") if item.strip()
        ]
        output = optional_path("OUMUQ_ONNX_VITS_OUTPUT") or (PROJECT_DIR / "onnx-vits-output").resolve()
        return cls(
            model_dir=optional_path("OUMUQ_ONNX_VITS_MODEL_DIR"),
            config_path=optional_path("OUMUQ_ONNX_VITS_CONFIG"),
            output_root=output,
            providers=raw_providers or None,
            default_speaker=os.environ.get("OUMUQ_ONNX_VITS_DEFAULT_SPEAKER", "").strip() or None,
            default_speaker_id=optional_int("OUMUQ_ONNX_VITS_DEFAULT_SPEAKER_ID"),
            default_speed=float(os.environ.get("OUMUQ_ONNX_VITS_DEFAULT_SPEED", "1")),
            max_seconds=float(os.environ.get("OUMUQ_ONNX_VITS_MAX_SECONDS", "120")),
        )
...
def default_engine_factory(settings: WorkerSettings) -> OnnxVitsEngine:
    if settings.model_dir is None or settings.config_path is None:
        raise RuntimeError("OUMUQ_ONNX_VITS_MODEL_DIR and OUMUQ_ONNX_VITS_CONFIG are required.")
...
class OnnxVitsWorker:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            return
        self._thread = threading.Thread(target=self._run_jobs, name="oumuq-onnx-vits", daemon=True)
        self._thread.start()
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "playback_host": "OumuQ route layer",
    ...
    def _run_job(self, job_id: str) -> None:
        ...
        try:
            ...
            metadata = {
                **result.metadata,
                "playback_delegated_to_oumuq": True,
...
def create_app(service: OnnxVitsWorker | None = None) -> FastAPI:
    ...
    api = FastAPI(title="OumuQ ONNX-VITS Worker", lifespan=lifespan)
